        ...
        if (!tableExists) {
            ...
            await pool.query(`
                CREATE TABLE IF NOT EXISTS employees (
                    ...
                    emp_email VARCHAR(100) NOT NULL CHECK (emp_email ~ '^[a-zA-Z][a-zA-Z0-9._-]{1,}[a-zA-Z]@astrolitetech\.com$')
                );
...
app.post('/api/appraisals', async (req, res) => {
    ...
    try {
        // Check if emp_id exists in employees table
        const empCheck = await pool.query('SELECT 1 FROM employees WHERE emp_id = $1', [empId]);
        if (empCheck.rowCount === 0) {
            // Insert a placeholder employee if not exists (for testing purposes)
            await pool.query(
                'INSERT INTO employees (emp_id, emp_name, emp_email) VALUES ($1, $2, $3)',
                [empId, empName, `${empName.toLowerCase().replace(/\s+/g, '.')}@astrolitetech.com`]
            );
        }
