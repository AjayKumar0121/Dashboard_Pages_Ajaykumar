    ...
    try {
      ...
      if (data.type === 'employeeDetails') {
        ...
        const personnelResult = await pool.query(
          'SELECT emp_id, name, email, job_role, location, department, hire_date, phone FROM personnel WHERE LOWER(email) = LOWER($1)',
          [email]
        );
        const personnelData = personnelResult.rows.length > 0 ? personnelResult.rows[0] : data.details;
        ws.send(JSON.stringify({
          type: 'employeeDetails',
          details: personnelData
        }));
        logger.debug('Sent personnel data to client:', personnelData);
      }
    } catch (error) {
...
const verifyToken = (req, res, next) => {
  ...
  try {
    ...
    logger.error('Token verification failed:', {
      ...
      // token: typeof token === 'string' ? `${token.slice(0, 10)}...` : 'invalid-token',
      path: req.path
    });
...
app.post('/check-email-data', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT id FROM user_accounts WHERE LOWER(email) = LOWER($1)', [email]);
    logger.debug('Check email query executed', { email, found: result.rows.length > 0 });
...
app.post('/api/signup', upload.single('profileImage'), async (req, res) => {
  ...
  try {
    const userExists = await pool.query(
      'SELECT id FROM user_accounts WHERE LOWER(email) = LOWER($1) OR username = $2',
      [email, username]
    );
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'SELECT id, username, email, password, profile_image FROM user_accounts WHERE LOWER(email) = LOWER($1)',
      [email]
    );
    ...
    try {
      ...
      logger.debug('Personnel data fetched for login', { email, found: !!personnelData });
    } catch (personnelError) {
      logger.warn('Failed to fetch personnel data during login:', {
        error: personnelError.message,
        stack: personnelError.stack,
        email,
        userId: user.id
      });
    }
...
app.get('/api/personnel', verifyToken, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      logger.warn('Personnel details not found', { email });
      return res.status(404).json({ error: 'Personnel details not found' });
    }

    logger.debug('Personnel data fetched', { email, data: result.rows[0] });
    res.json({
...
app.get('/api/profile', verifyToken, async (req, res) => {
  try {
    ...
    logger.debug('User profile fetched', { userId, email: user.email });
    ...
    try {
      ...
      logger.debug('Personnel query executed', {
        email: user.email,
        found: personnelResult.rows.length > 0,
        data: personnelData
      });
    } catch (personnelError) {
    ...
    logger.info('Profile data sent to client', { userId, email: user.email, hasPersonnel: !!personnelData });
    res.json({
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      logger.warn(`Password reset attempt for non-existent email: ${email}`, { ip: req.ip });
    ...
    logger.debug(`Password reset token generated for ${email}`, { userId: result.rows[0].id });
...
app.get('/api/debug-token', (req, res) => {
  res.json({
    cookieToken: req.cookies.accessToken || null,
