document.addEventListener('DOMContentLoaded', async () => {
 ...
 dostęp: false,
  ...
  const utils = {
    ...
    handleApiError: (error) => {
      ...
      if (error.status === 401) {
        core.verifySession(); // Attempt to refresh and re-verify
      }
    },
 ...
 Angielski
        ...
        if (!response.ok) {
          ...
          if (response.status === 401 && state.refreshToken) {
            ...
            if (refreshResponse.ok) {
              ...
              document.cookie = `accessToken=${accessToken}; maxAge=900; path=/`; // 15 minutes
  ...
  const core = {
    ...
    verifySession: async () => {
      ...
      try {
        ...
        state.personnelDetails = response.personnel || state.personnelDetails || null; // Fallback to existing state.personnelDetails

        // Store personnel details in localStorage if available
        if (state.personnelDetails) {
          localStorage.setItem('abccompanyempdetails', JSON.stringify(state.personnelDetails));
        } else {
          // If no personnel details from API, try to retrieve from localStorage
          const storedPersonnelDetails = localStorage.getItem('abccompanyempdetails');
          if (storedPersonnelDetails) {
            try {
              state.personnelDetails = JSON.parse(storedPersonnelDetails);
            } catch (e) {
              console.error('Failed to parse abccompanyempdetails:', e);
        ...
        if (refreshToken && error.status === 401) {
          try {
            ...
            if (refreshResponse.ok) {
              ...
              document.cookie = `accessToken=${accessToken}; maxAge=900; path=/`; // 15 minutes
              state.token = accessToken;
              return await core.verifySession(); // Retry with new token
            }
          } catch (refreshError) {
    ...
    handleLogin: async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          if (data.accessToken) {
            document.cookie = `accessToken=${data.accessToken}; maxAge=900; path=/`; // 15 minutes
            state.token = data.accessToken;
          }
          if (data.refreshToken) {
            document.cookie = `refreshToken=${data.refreshToken}; maxAge=604800; path=/`; // 7 days
  ...
  const init = async () => {
    // Retrieve stored personnel details from localStorage
    const storedPersonnelDetails = localStorage.getItem('abccompanyempdetails');
    if (storedPersonnelDetails) {
      try {
        state.personnelDetails = JSON.parse(storedPersonnelDetails);
